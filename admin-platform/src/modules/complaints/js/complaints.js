$(function () {
    ...
    $(".add_note_complaint").on("click", function (e) {
      e.preventDefault();
      ...
      $.post(admin_url + "misc/add_note/" + complaintid + "/complaint", {
    ...
    $(".save_changes_settings_single_complaint").on("click", function (e) {
      e.preventDefault();
      var data = {};
      ...
      data += "&complaintid=" + $('input[name="complaintid"]').val();
      if (typeof csrfData !== "undefined") {
        data += "&" + csrfData["token_name"] + "=" + csrfData["hash"];
      }
      $.post(admin_url + "complaints/update_single_complaint_settings", data).done(
        function (response) {
          response = JSON.parse(response);
          if (response.success == true) {
            if (typeof response.department_reassigned !== "undefined") {
              window.location.href = admin_url + "complaints/";
            } else {
              window.location.reload();
            }
    ...
    $('select[name="status_top"]').on("change", function () {
      ...
      requestGetJSON(
        "complaints/change_status_ajax/" + ticketid + "/" + status
    ...
    $('body.ticket select[name="contactid"]').on("change", function () {
      ...
      if (contactid != "") {
        $.post(admin_url + "complaints/complaint_change_data/", {
  ...
  function validate_new_complaint_form() {
    ...
    setTimeout(function () {
      $.each(
        $("#new_complaint_form").find('[data-custom-field-required="1"]'),
        function () {
