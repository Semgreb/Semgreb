...
class SSP
{
    ...
    static function limit_sp($request)
    {
        $pagina = 0;

        $pagina = $request['start'] / $request['length'];
        $pagina++;

        return array("pagina" => $pagina, "cantidad" => $request['length']);
    }



    static function filter_sp($request, $columns)
    {
        $globalSearch = array();
        $columnSearch = array();
    ...
    static function simple_sp($request, &$data, $columns)
    {
        // Build the SQL query string from the request
        //$order = self::order( $request, $columns );

        $recordsFiltered = $data[1][0]['sLINEAS'];


        $recordsTotal = $data[1][0]['sLINEAS'];
        //        $recordsTotal = $data[2][0]['sTOTAL'];
        ...
        return array(
            ...
            "data" => self::data_output($columns, $data[0])
        );
    }
    ...
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        if ($joinQuery) {
            if ($extraWhere && $where) {
                $extraWhere = ' WHERE ' . substr($extraWhere, 4);
            }
            $count_request .= $joinQuery . $extraWhere;
        } else {
    ...
    static function assign_conditions_query($extraWhere, $yesQuery, $noQuery)
    {
        if ($extraWhere != "") {
            $extraWhere .= $yesQuery;
        } else {
